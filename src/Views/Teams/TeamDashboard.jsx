...
import { FaFlagCheckered, FaTrophy, FaUsers, FaUser, FaPlus, FaEdit, FaSave, FaComments, FaTrash, FaTimes, FaArrowLeft, FaCheck, FaChevronRight } from 'react-icons/fa';
...
const SectionHeader = ({ icon, title, actionText, onAction }) => (
    <div className="flex items-center justify-between mb-3">
        ...
        {actionText && (
            <button className="text-[#EE8631] text-xs flex items-center" onClick={onAction}>
                {actionText} {actionText && <FaChevronRight className="ml-1" size={10} />}
            </button>
        )}
    </div>
);
                ...
                <div className="space-y-4">
                    ...
                    <div className="bg-[#35383f] rounded-xl border border-[#95C5C5]/10 p-4 shadow-lg">
                        ...
                        <div className="bg-[#292B35] rounded-lg p-4 mb-4">
                            <p className="text-sm text-[#E0E0E0] mb-3">{team.TEAM_DESCRIPTION || "No team description available."}</p>
                        ...
                        <div className="space-y-2 max-h-[250px] overflow-y-auto pr-1">
                            {teamMembers.length > 0 ? (
                                teamMembers.map((member, index) => (
                                    <div key={index} className="flex items-center p-3 bg-[#292B35] rounded-lg">
                                        <FaUser className={member.ACCESS === 'ADMIN' ? "text-[#EE8631]" : "text-[#95C5C5]"} />
                                ...
                                <div className="text-[#95C5C5]/70 italic text-center py-3">No members</div>
                            )}
                            {isAdmin && (
                ...
                <div className="space-y-4">
                    <div className="bg-[#35383f] rounded-xl border border-[#95C5C5]/10 p-4 shadow-lg">
                        <SectionHeader
                            ...
                            actionText={isAdmin ? (showMilestoneForm ? "Cancel" : "Add New") : null}
                        ...
                        <div className="space-y-2 max-h-[320px] overflow-y-auto pr-1">
                            {milestones.length > 0 ? (
                                milestones.map(m => (
                                    <div key={m.MILESTONE_ID} className="flex items-center p-3 bg-[#292B35] rounded-lg">
                                ...
                                <div className="text-center py-8 text-gray-500">
                                    <FaFlagCheckered size={24} className="mx-auto mb-2 opacity-50" />
                            ...
                            <div className="flex-1 bg-[#292B35] rounded-lg p-4 mb-4 overflow-y-auto">
                                {chatMessages.map(msg => (
                                    <div key={msg.id} className="mb-2">
                                        <span className="font-bold text-[#95C5C5]">{msg.user}:</span>
                            ...
                            {team.EVENTS_ENROLLED && team.EVENTS_ENROLLED.length > 0 ? (
                                team.EVENTS_ENROLLED.map(e => (
                                    <div key={e.EVENT_ID} className="bg-[#292B35] p-4 rounded-lg flex flex-col">
                                        ...
                                        <div className="text-[#E0E0E0] text-sm">{e.DESCRIPTION}</div>
