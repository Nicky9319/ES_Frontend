            ...
            <div className="relative h-[38vh] md:h-[44vh] flex items-center justify-center bg-gradient-to-br from-[#23242b] via-[#292B35] to-[#1a1b20] overflow-hidden mb-10">
                ...
                <div className="relative z-10 text-center">
                    ...
                    <p className="text-lg md:text-xl text-[#E0E0E0]/80 mb-4 font-medium">
                        Discover, filter, and join the most exciting tournaments in the esports world.
                    </p>
                    <button
            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex flex-col md:flex-row md:items-center gap-4 mb-8">
                    ...
                    <div className="flex items-center gap-2 bg-[#23242b] px-4 py-3 rounded-lg border border-[#95C5C5]/20">
                        ...
                        <span className="text-[#95C5C5]"><FaSort /></span>
                    </div>
                    <div className="flex items-center gap-1 bg-[#23242b] p-1 rounded-lg border border-[#95C5C5]/20">
                        <button
                            onClick={() => setViewMode('grid')}
                            className={`p-2 rounded ${viewMode === 'grid' ? 'bg-[#95C5C5] text-[#292B35]' : 'text-[#E0E0E0] hover:bg-[#292B35]'}`}
                            title="Grid View"
                        >
                            <FaTh />
                        </button>
                        <button
                            ...
                            className={`p-2 rounded ${viewMode === 'list' ? 'bg-[#95C5C5] text-[#292B35]' : 'text-[#E0E0E0] hover:bg-[#292B35]'}`}
                            title="List View"
                        >
                            <FaList />
                        </button>
                ...
                <div className={`transition-all duration-300 ${showFilters ? 'block' : 'hidden md:block'} mb-8`}>
                ...
                <div className={`grid gap-8 ${viewMode === 'grid' ? 'grid-cols-1 md:grid-cols-2 xl:grid-cols-3' : 'grid-cols-1'}`}>
                    {isLoading ? (
                        <div className="col-span-full text-[#95C5C5] text-center py-16 text-xl">Loading events...</div>
                    ) : filteredAndSortedEvents.length === 0 ? (
                        <div className="col-span-full text-[#95C5C5] text-center py-16 text-xl">No events found</div>
                    ) : (
                        filteredAndSortedEvents.map(event => (
                            <div
                                key={event.id}
                                className={`relative bg-[#23242b] rounded-2xl shadow-xl border border-[#95C5C5]/10 overflow-hidden hover:shadow-2xl transition-all duration-300 flex flex-col ${viewMode === 'list' ? 'md:flex-row' : ''}`}
                            >
                                <div className={`${viewMode === 'list' ? 'md:w-1/3' : 'h-56'} overflow-hidden relative`}>
                                    <img
                                        src={event.image}
                                        alt={event.title}
                                        className={`w-full h-full object-cover transition-transform duration-500 ${viewMode === 'list' ? 'md:h-full' : 'hover:scale-105'}`}
                                ...
                                <div className={`flex-1 p-6 flex flex-col justify-between`}>
                                    <div>
                                        <div className="flex flex-wrap justify-between items-start mb-3 gap-2">
                                            <h2 className="text-2xl font-bold text-[#95C5C5] hover:text-[#EE8631] transition-colors duration-300">{event.title}</h2>
                                        ...
                                        <div className="flex flex-wrap gap-4 mb-4 text-[#E0E0E0]/90 text-sm">
                                            <div className="flex items-center gap-2">
                                                <FaCalendarAlt className="text-[#EE8631]" /> {event.date.toLocaleDateString()}
                                            </div>
                                            <div className="flex items-center gap-2">
                                                <FaMapMarkerAlt className="text-[#EE8631]" /> {event.venue}
                                            </div>
                                            <div className="flex items-center gap-2">
                                                <FaGamepad className="text-[#EE8631]" /> {event.format}
                                            </div>
                                        </div>
                                        <p className="text-[#E0E0E0]/80 mb-4 line-clamp-3">{event.description}</p>
                                    </div>
